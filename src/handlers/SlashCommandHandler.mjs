...
import { cooldownCategories, cooldownCategoriesHigh, cooldownCommands, defaultCooldownMs, defaultCooldownMsHigh, maximumCoolDownCommands } from "../data/Cooldowns.mjs";
import { onlySecondDuration } from "../utils/TimeUtils.mjs";

/** 
 * @param {import("../structures/Bready.mjs").Bready} client
...
export function isOnCooldown(client, command, ctx) {
    ...
    const defaultCooldown =
        cooldownCategoriesHigh.includes(command.category)
    ...
    if(command.cooldown?.user) {
        ...
        if(commandCooldown > Date.now()) {
            return ctx.reply({
                ephemeral: true,
                embeds: [
                    new ErrorEmbed(ctx).addField(`${Emoji(ctx).Cooldown.str} Calm,`, `> You can use this Command \`${onlySecondDuration(commandCooldown - Date.now())}\``)
                ],
            }).catch(() => null), true;
        }
    ...
    if(command.cooldown?.guild ?? defaultCooldown) {
        ...
        if(commandCooldown > Date.now()) {
            return ctx.reply({
                ephemeral: true,
                embeds: [
                    new ErrorEmbed(ctx).addField(`${Emoji(ctx).Cooldown.str} Calm`, `> This Server can use this Command \`${onlySecondDuration(commandCooldown - Date.now())}\``)
                ],
            }).catch(() => null), true;
        }
    ...
    if(allCools.length > maximumCoolDownCommands.amount) {
        return ctx.reply({
            ephemeral: true,
            embeds: [
                new ErrorEmbed(ctx).addField(`${Emoji(ctx).Cooldown.str} Calm`, `> You only get to use ${maximumCoolDownCommands.amount} Commands per ${maximumCoolDownCommands.time / 1000} Seconds`)
            ],
        }).catch(() => null), true;
    }
    return false;
