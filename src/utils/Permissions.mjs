...
export function returnOverwrites(client, channel) {
  ...
  const overwrites =
    channel?.overwritesFor?.(me, true, roles) ||
    getChannelOverwrites(me, channel, roles);

  if (overwrites.everyone?.deny)
    permissions = permissions.remove(overwrites.everyone?.deny);
  if (overwrites.everyone?.allow)
    permissions = permissions.add(overwrites.everyone?.allow);
  if (overwrites.roles.length > 0)
    permissions = permissions.remove(overwrites.roles.map((role) => role.deny));
  if (overwrites.roles.length > 0)
    permissions = permissions.add(overwrites.roles.map((role) => role.allow));
  if (overwrites.member?.deny)
    permissions = permissions.remove(overwrites.member?.deny);
  if (overwrites.member?.allow)
    permissions = permissions.add(overwrites.member?.allow);
  return permissions;
}
...
export function checkPerms(client, channel, ...PermissionFlagsBits) {
  ...
  if (channel?.guild?.members?.me)
    return checkPermOverwrites(client, channel, ...PermissionFlagsBits);
  return channel
    ?.permissionsFor?.(client.user.id)
    ?.has?.([...PermissionFlagsBits.flat()]);
}
